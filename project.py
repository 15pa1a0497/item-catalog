...
from database_setup import Base, Brand, Bag, User
...
engine = create_engine('sqlite:///bag.db')
...
@app.route('/brand/<int:brand_id>/Bag/JSON')
def brandProductJSON(brand_id):
    brand = session.query(Brand).filter_by(id=brand_id).one()
    details = session.query(Bag).filter_by(
        brand_id=brand_id).all()
    return jsonify(Bag=[i.serialize for i in details])


@app.route('/brand/<int:brand_id>/details/<int:details_id>/JSON')
def productsJSON(brand_id, details_id):
    Product_Details = session.query(Bag).filter_by(id=details_id).one()
...
@app.route('/brand/<int:brand_id>/Bag/')
def showBag(brand_id):
    session5 = DBSession()
    brand = session5.query(Brand).filter_by(id=brand_id).one()
    details = session5.query(Bag).filter_by(brand_id=brand_id).all()
    session5.close()
    for d in details:
        print(d.name)
    return render_template('product.html', details=details, brand=brand)
    # return 'This page is the Bag for brand %s' % brand_id

# Create a new Bag details


@app.route(
    '/brand/<int:brand_id>/Bag/new/', methods=['GET', 'POST'])
def newBag(brand_id):
    session6 = DBSession()
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if login_session['user_id'] != brand.user_id:
        if deleteBrand.user_id != login_session['user_id']:
            ...
            to edit/delete.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        newBag = Bag(name=request.form['name'],
        ...
        return redirect(url_for('showBag', brand_id=brand_id))
    else:
        session6.close()
        return render_template('newProduct.html', brand_id=brand_id)

    return render_template('newProduct.html', brand=brand)
    # return 'This page is for making a new Bag details for brand %s'
    # %brand_id

# Edit a Bag details


@app.route('/brand/<int:brand_id>/Bag/<int:product_id>/edit',
           methods=['GET', 'POST'])
def editBag(brand_id, product_id):
    session7 = DBSession()
    if 'username' not in login_session:
        return redirect('/login')
    editBag = session7.query(Bag).filter_by(id=product_id).one()
    ...
    if request.method == 'POST':
        ...
        if request.form['description']:
            editBag.description = request.form['name']
        if request.form['price']:
            editBag.price = request.form['price']
        if request.form['material']:
            editBag.material = request.form['material']
        session7.add(editBag)
        session7.commit()
        session7.close()
        return redirect(url_for('showBag', brand_id=brand_id))
    else:
        ...
        return render_template('editProduct.html', brand_id=brand_id,
                               product_id=product_id, details=editBag)

    # return 'This page is for editing Bag details %s' % product_id

# Delete a Bag details


@app.route('/brand/<int:brand_id>/Bag/<int:product_id>/delete',
           methods=['GET', 'POST'])
def deleteBag(brand_id, product_id):
    session8 = DBSession()
    if 'username' not in login_session:
        return redirect('/login')
    editBag = session8.query(Bag).filter_by(id=product_id).one()
    brand = session8.query(Brand).filter_by(id=brand_id).one()
    deleteBag = session8.query(Bag).filter_by(id=product_id).one()
    if login_session['user_id'] != brand.user_id:
        if deleteBrand.user_id != login_session['user_id']:
            return "<script>function myFunction() {alert('You \
            are not authorized to delete this Brand.\
            ...
            to edit/delete.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        session8.delete(deleteBag)
        session8.commit()
        session8.close()
        return redirect(url_for('showBag', brand_id=brand_id))
    else:
        session8.close()
        return render_template('deleteProduct.html', details=deleteBag)
    # return "This page is for deleting Bag details %s" % product_id
